...
function makeSafeResult(result) {
  ...
  return {
    ...
    contact: result.contact || { name: "", contact: "", email: "", address: "" },
    socialmedia: result.socialmedia || { instagram: "", linkedin: "", github: "", facebook: "" },
...
router.get("/dashboard",isLoggedIn,async(req,res)=>{
  ...
  res.render("./pages/admin/dashboard.ejs",{currentPage: "dashboard", url : req.originalUrl, data})
})
...
router.get("/position",isLoggedIn,async(req,res)=>{
    ...
    data = makeSafeResult(data);
    console.log(data);
  res.render("./pages/admin/dashboard.ejs",{currentPage: "dashboard",url : parts.pop(),data : data})
})
...
router.post("/experience/:id",isLoggedIn,async(req,res)=>{
    ...
    await portfolioModel.findByIdAndUpdate(id.id, { $push : { experience : { company, designation, description, DOJ : doj, DOL : dol }}}, { new: true });
  res.redirect("/atul-admin/experience")
  // res.send(req.body)
})

// experience view
router.get("/portfolio/:protid/experience/:expid",isLoggedIn, async (req, res) => {
    try {
...
router.post("/experience/:id",isLoggedIn,async(req,res)=>{
    ...
    await portfolioModel.findByIdAndUpdate(id.id, { $push : { experience : { company, description, DOJ : doj, DOL : dol }}}, { new: true });
  res.redirect("/atul-admin/experience")
  // res.send(req.body)
})
...
// experience delete
router.post("/experience/delete/:protid/:expid", isLoggedIn, async (req, res) => {
...
router.post("/education/:id",isLoggedIn,async(req,res)=>{
    ...
    await portfolioModel.findByIdAndUpdate(id.id, { $push : {
      education : {
        insitute, course, grade, from, to
      }
    } },{new : true});
...
// delete education
// experience delete
router.post("/education/delete/:protid/:eduid", isLoggedIn, async (req, res) => {
...
router.post("/projects/:id",isLoggedIn,async(req,res)=>{
    // const parts = req.originalUrl.split('/');
    const id = req.params;
    let {projectname,description,technologies} = req.body;
...
router.get("/projects/edit/:portid/:projid",isLoggedIn,async (req,res)=>{
    ...
    try {
        let portfolio = await portfolioModel.findOne({
            _id : portid, "projects._id" : projid
        },
    {
        "projects.$" : 1
    });
...
// delete project
router.post("/projects/delete/:portid/:projid",isLoggedIn,async(req,res)=>{
...
router.put("/contact/:projectid",isLoggedIn,async(req,res)=>{
    ...
    try {
            const data =await portfolioModel.findByIdAndUpdate(req.params.projectid,{
        $set : {
            contact : {
                name,
                contact,
...
router.put("/skillexpertise/:id",isLoggedIn,async(req,res)=>{
    // const parts = req.originalUrl.split('/');
    const id = req.params;
    const {languages, webtech, frameworkLibrary, databases, toolsplateforms, professionalskills} = req.body;
    await portfolioModel.findByIdAndUpdate(id.id, { $set :{ skillexpertise : {
languages, webtech, frameworkLibrary, databases, toolsplateforms, professionalskills
    }} }, { new : true});
  res.redirect("/atul-admin/dashboard")
})
